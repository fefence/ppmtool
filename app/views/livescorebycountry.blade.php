            ...
            @foreach($matches as $m)
                <tr id="{{$m->id}}">
                    ...
                    <td>
                        ...
                        <a href="/play/{{date('Y-m-d', strtotime($m->date_time))}}/{{date('Y-m-d', strtotime($m->date_time))}}/#{{$league}}" role="button" class="btn @if($m->home_goals.'-'.$m->away_goals == $s->game_type->name && $m->short_result != '-') btn-success @else btn-info @endif btn-xs hasTooltip" title="{{$s->s}}" @if($s->s == 0) disabled @endif>{{$s->game_type->name}}</a>
                        @endforeach
                    </td>

...
@if(array_get($data, 'team') == '')
    <a href="#" type="button" data-toggle="tooltip" data-placement="top" title="<strong>{{array_get($data, 'match')->home_goals}}:{{array_get($data, 'match')->away_goals}}</strong>&nbsp;({{array_get($data, 'match')->home}}&nbsp;-&nbsp;{{array_get($data, 'match')->away}})<br/>{{ date("d.m.Y",strtotime(array_get($data, 'match')->date_time)) }}" class="btn btn-info hasTooltip
    @if(isset($game_type))
        ...
        @elseif(array_get($data, 'match')->short_result == '-' || array_get($data, 'match')->state == 'Canceled')
            btn-info btn-xs">{{array_get($data, 'match')->short_result}}
        @endif
    @else
            {{"btn-info"}}
        btn-xs w25">{{array_get($data, 'match')->short_result}}
    @endif
    </a>
@else
<a href="#" type="button" data-toggle="tooltip" data-placement="top" title="<strong>{{array_get($data, 'match')->home_goals}}:{{array_get($data, 'match')->away_goals}}</strong>&nbsp;({{array_get($data, 'match')->home}}&nbsp;-&nbsp;{{array_get($data, 'match')->away}})<br/>{{ date("d.m.Y",strtotime(array_get($data, 'match')->date_time)) }}"
    ...
    {{'class="btn btn-xs btn-info hasTooltip">D'}}
    ...
    {{'class="btnbtn-xs btn-info hasTooltip">W'}}
    ...
    {{'class="btn btn-info btn-xs hasTooltip">L'}}
    @else
    {{'class="btn btn-info btn-xs hasTooltip">?'}}
    @endif
</a>
@endif